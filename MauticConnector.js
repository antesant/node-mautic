...
class MauticConnector {
    ...
    _convertQueryParametersToString(queryParameters = {}) {
        ...
        return Object.keys(queryParameters).map(key => key + '=' + (Array.isArray(queryParameters[key]) ? queryParameters[key].map(p => encodeURIComponent(p)).join(',') : encodeURIComponent(queryParameters[key]))).join('&');
    }
